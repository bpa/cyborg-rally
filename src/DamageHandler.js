...
export default class DamageHandler extends Component {
    constructor(props) {
        super(props);
        var pending_shots = [];
        var disputed = [];
        if (GameContext.state) {
            for (var shot of GameContext.state.shots) {
                if (shot.dispute) {
                    if (shot.voted[GameContext.id] === undefined) {
                        disputed.push(shot);
                    }
                }
                else if (shot.target === GameContext.id) {
